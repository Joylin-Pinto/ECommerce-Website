...
namespace Project
{
    public partial class EditSize : System.Web.UI.Page
    {
        ...
        {
            SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["ProjectDB"].ConnectionString);
            if (con.State == ConnectionState.Closed) { con.Open(); }
            SqlCommand cmd = new SqlCommand("select SizeName,BrandID,CategoryId,SubCategory,GenderId from tblSize where SizeID=@ID", con);
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                SubCID = ds.Tables[0].Rows[0]["SubCategory"].ToString();
                subcategory();
        ...
        private void BindGridview()
        {
            SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["ProjectDB"].ConnectionString);
            if (con.State == ConnectionState.Closed) { con.Open(); }
            SqlDataAdapter da = new SqlDataAdapter("select t1.SizeID,t1.SizeName,t2.Name as Brand,t3.CatName as Category,t4.SubCatName as SubCategory,t5.GenderName as Gender from tblSize as t1 with(nolock) inner join tblBrands as t2 with(nolock) on t2.BrandID=t1.BrandID inner join tblCategory as t3 with(nolock) on t3.CatID=t1.CategoryID inner join tblSubCategory as t4 with(nolock) on t4.SubCatID=t1.SubCategory inner join tblGender as t5 with(nolock) on t5.GenderID=t1.GenderId", con);
            DataTable dt = new DataTable();
            da.Fill(dt);
            con.Close();
            if (dt.Rows.Count > 0)
        ...
        protected void btnUpdateSubCategory_Click(object sender, EventArgs e)
        {
            if (txtID.Text != string.Empty && txtSize.Text != string.Empty && ddlCategory.SelectedIndex != -1)
            {
                SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["ProjectDB"].ConnectionString);
                if (con.State == ConnectionState.Closed) { con.Open(); }
                SqlCommand cmd = new SqlCommand("update tblSize set SizeName=@SizeName,BrandID=@BrandID,CategoryId=@CategoryID,SubCategory=@SubCategoryID,GenderId=@GenderID where SizeID=@SizeID", con);
